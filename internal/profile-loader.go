...
func getAllProfiles(awsConfigFiles *ini.File) []string {
    allProfileNamesRaw := awsConfigFiles.SectionStrings()

    var allProfileNames []string

    for _, profileName := range allProfileNamesRaw {
        allProfileNames = append(allProfileNames, strings.Replace(profileName, "profile ", "", 1))
    }
...
func getProfileSectionName(awsConfigFiles *ini.File, profileName string) string {
    ...
    for _, name := range allProfileSections {
        if name == profileName || name == fmt.Sprintf("profile %s", profileName) {
            return name
        }
    }
